...
public class BlurMakeSystem : ILoadable
{
    ...
    /// </summary>
    public static bool SingleBlur { get; set; } = false;
    ...
    public static float BlurZoomMultiplierDenominator { get; set => field = Math.Max(value, 1f); } = 2f;
    ...
    public void Load(Mod mod)
    {
        ...
        On_Main.DrawInterface += static (orig, self, gameTime) =>
        {
            if (BlurAvailable)
            {
                ...
                else ApplyBlur([Main.skyTarget, Main.screenTarget]);
            }
    ...
    private static void PrepareRenderTargets()
    {
        ...
        if (SingleBlur)
        {
            UserInterfaceRenderTarget = GetOrRecreateTarget(UserInterfaceRenderTarget, sourceWidth, sourceHeight);
    ...
    /// </summary>
    public static void ApplyBlur(RenderTarget2D[] source)
    {
